    ...
    try {
      if (user?.company_id) {
        await buyerApi.createBuyingRequest(user.company_id, {
          ...
          target_quantity: Number(targetVolumeKg) / 1000.0,
          unit: 'ton',
        ...
        <div style={{ backgroundColor: '#ffffff', border: '1px solid var(--card-border)', borderRadius: '14px', padding: '1.75rem', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
          <div style={{ borderBottom: '1px solid var(--card-border)', paddingBottom: '0.75rem', marginBottom: '1.25rem' }}>
            <h2 style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--text-dark)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <FactoryIcon size={20} color="var(--primary-blue)" /> Form Kebutuhan Rutin Biomassa
            </h2>
          ...
          <form onSubmit={handleSaveDemand} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
            ...
            {formSuccess && (
              <div style={{ fontSize: '0.82rem', color: '#047857', backgroundColor: '#ecfdf5', border: '1px solid #a7f3d0', padding: '0.65rem 0.85rem', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <CheckCircleIcon size={16} color="#047857" /> {formSuccess}
              </div>
            )}
