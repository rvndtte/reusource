...
function MainApp() {
  ...
  // Buyer Sub-Tab Selector: 'katalog' | 'permintaan'
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: 'var(--bg-eco)' }}>
      ...
      <main style={{ overflow: 'hidden' }}>
        ...
        {isAuthenticated && isSupplier && (
          <ProtectedRoute allowedRoles={['supplier_admin', 'supplier']} requiredRoleName="Pemasok (Supplier)" onOpenAuthModal={openAuthModal}>
            <SupplierDashboard />
          </ProtectedRoute>
        )}
        ...
        {isAuthenticated && isBuyer && (
          <ProtectedRoute allowedRoles={['buyer_admin', 'buyer']} requiredRoleName="Pembeli (Buyer)" onOpenAuthModal={openAuthModal}>
            {buyerTab === 'katalog' && <BuyerKatalog />}
            {buyerTab === 'permintaan' && <BuyerPermintaan />}
          </ProtectedRoute>
        )}

        {/* 3. ADMIN CONSOLE (Protected for Admin / Verifier Role) */}
        {isAuthenticated && isAdmin && (
          <ProtectedRoute allowedRoles={['admin', 'verifier']} requiredRoleName="Administrator / Verifier" onOpenAuthModal={openAuthModal}>
            <AdminVerifikasi />
          </ProtectedRoute>
        )}
