...
export default function FactsAndImpact() {
  ...
  return (
    <section id="fakta-sdg-section" style={{
      maxWidth: '1280px',
      margin: '0 auto',
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
          <div>
            ...
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600, marginTop: '2px' }}>
              Berdasarkan rujukan standar industri resmi &amp; penelitian terapan
            </p>
          </div>
          <button
            type="button"
          ...
          {REFERENCES_DETAIL.map((item, i) => (
            <div key={i} style={{
              ...
              <div style={{ fontSize: '0.66rem', color: 'var(--primary-green)', fontWeight: 700, paddingTop: '0.4rem', borderTop: '1px dashed #e2e8f0', width: '100%' }}>
                Rujukan: {item.links.map((l, idx) => <a key={idx} href={l.url} target="_blank" rel="noopener noreferrer" style={{ color: '#059669', textDecoration: 'underline' }}>{l.name.split(' ')[0]}</a>).reduce((prev, curr) => [prev, ' • ', curr])}
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Container 2: Dampak Nyata, Selaras SDG */}
      <div style={{
        backgroundColor: '#ffffff',
        border: '1px solid var(--card-border)',
        borderRadius: '20px',
